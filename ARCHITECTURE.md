...
- **`yidomon0@gmail.com`** (compte locataire rattaché à DEUX organisations — "RIMCO" et "Agence Demo" — via `tenant_organization_memberships`, utilisé pour diagnostiquer l'affichage `/tenant` en contexte multi-organisation). Mot de passe réinitialisé via l'API Admin à plusieurs reprises (mot de passe d'origine inconnu, non récupérable ; dernière réinitialisation le 2026-08-27) — mot de passe actuel : `TenantMultiOrg2026!0b8b7e`. Porte désormais un bail actif + un ticket de maintenance (`signale`) dans CHACUNE des deux organisations (biens "Studio Test Multi-Org" / "Studio Test Multi-Org (Agence Demo)", tous deux créés pour ce diagnostic, dépôts à 0 pour activer sans écriture dans `deposit_ledger`) — plus "aucun bail/ticket" comme indiqué précédemment.
- **`rimco@gmil.com`** (compte admin de l'organisation "RIMCO", seul compte interne de cette organisation). Mot de passe réinitialisé via l'API Admin le 2026-08-26 (mot de passe d'origine inconnu, non récupérable) — mot de passe actuel : `RimcoAdminMultiOrg2026!136bb1`.
